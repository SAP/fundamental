...
const paths = {
    ...
    dest: (environment.production ? config.root.dest : config.root.tmp) + '/theme/ariba'
}
...
gulp.task('pkg-font-icon-css-ariba', fontIconTask);

// Copy Font-icon files
const taskCopyFontIconFiles = (cb) => {
    return gulp.src([`${paths.src}/icons/*.woff`])
        .pipe(gulp.dest(paths.dest));
}
gulp.task('pkg-font-icon-ariba', taskCopyFontIconFiles);
...
module.exports = cssTask = (cb) => {
    gulpSequence(
        ...
        'pkg-font-icon-css-ariba',
        'pkg-font-icon-ariba',
        'pkg-css-minify-ariba',
        'pkg-css-banner-ariba',
        cb)
}
gulp.task('pkg-theme-ariba', cssTask);
